...
(function ($) {
  ...
  Spoon.GroupsDesigner = Craft.FieldLayoutDesigner.extend({
    ...
    initElement: function initElement($element) {
      // Base init
      new Craft.FieldLayoutDesigner.Element(this, $element); // Add our settings button
    ...
    addTab: function addTab() {
      ...
      var $tab = $('<div class="fld-tab">' + '<div class="tabs">' + '<div class="tab sel draggable">' + '<span>Group ' + (this.tabGrid.$items.length + 1) + '</span>' + '<a class="settings icon" title="' + Craft.t('app', 'Rename') + '"></a>' + '</div>' + '</div>' + '<div class="fld-tabcontent"></div>' + '</div>').appendTo(this.$tabContainer);
     ...
     */
    onEditFieldLayout: function onEditFieldLayout($blockType, $btn) {
     ...
     */
    editFieldLayout: function editFieldLayout($blockType) {
      // Build the markup
        ...
        onFadeIn: $.proxy(function () {
          ...
          Craft.postActionRequest('spoon/configurator/get-fields-html', data, $.proxy(function (response, textStatus) {
            if (textStatus === 'success') {
              ...
              $bigSpinner.addClass('hidden'); // var fld = new Craft.FieldLayoutDesigner('#spoon-fields-configurator', {
    ...
    _handleSubmit: function _handleSubmit(ev) {
      ev.preventDefault(); // Show spinner

      this.$spinner.removeClass('hidden'); // Get the form data

      var data = this.$form.serialize(); // Post it
...
/******/ })()
;
//# sourceMappingURL=GroupsDesigner.js.map